...
if __name__ == '__main__':
    ...
    if args.overwrite:
        ...
        try:
            ...
            if start_r < args.repeat:
                ...
                for mkey in metric_keys: 
                    avg_metrics[mkey]['global'] = np.append(avg_metrics[mkey]['global'], np.zeros((max_task,args.repeat-start_r)), axis=-1) # : make this queal to number of clients
    ...
    for r in range(start_r, args.repeat):
        ...
        if r == 0: 
            for mkey in metric_keys: 
                avg_metrics[mkey]['global'] = np.zeros((max_task,args.n_clients*args.n_rounds)) 
                if (not (mkey in global_only)):
        ...
        if not args.only_eval_zs:
            avg_metrics = trainer.train(avg_metrics)  ##  : Jump to trainer.py
        # evaluate model
